# 00-story-asset-bank

## Asset 1. Legacy Migration Issue

- Summary: 오래된 시스템을 새 구조로 옮기면서 데이터가 맞지 않았던 경험
- Reusable topics: work, problem solving, past vs present, tech trend
- Tone: calm, practical
- Keywords: log, bug, migration, rollback, check

## Asset 2. Old vs New Work Style

- Summary: 예전에는 수동 작업이 많았고, 지금은 자동화와 AI 도구를 더 쓴다
- Reusable topics: work comparison, industry trend, social issue
- Tone: realistic
- Keywords: manual, automated, faster, easier

## Asset 3. Apartment Routine

- Summary: 퇴근 후 집에서 쉬고, 저녁에 가족과 15분 정도 산책한다
- Reusable topics: home, routine, housing issue
- Tone: warm, simple
- Keywords: living room, kitchen, walk, family

## Asset 4. Neighborhood Cafe

- Summary: 집 근처 조용한 카페에서 아메리카노를 마시며 30분 정도 쉬는 이야기
- Reusable topics: cafe, study, weekend routine
- Tone: relaxed
- Keywords: Americano, window seat, quiet, local

## Asset 5. Park Walk Loop

- Summary: 집 근처 공원 호수길을 20~30분 걷는 루틴
- Reusable topics: park, health, seasons, routine
- Tone: fresh, peaceful
- Keywords: lake, sneakers, evening, exercise

## Asset 6. Weekend Drive

- Summary: 주말에 가족과 근교로 드라이브를 가는 경험
- Reusable topics: drive, vacation, destination, problem solving
- Tone: easygoing
- Keywords: navigation, traffic, rest stop, fuel

## Asset 7. Music Playlist

- Summary: 출퇴근과 산책 때 듣는 잔잔한 팝/인디 플레이리스트
- Reusable topics: music, routine, trend, mood
- Tone: calm
- Keywords: playlist, earbuds, commute, mood

## Asset 8. Online Shopping Routine

- Summary: 온라인으로 생활용품, 노트북 액세서리, 아이 물건을 신중하게 사는 루틴
- Reusable topics: shopping, online shopping, comparison, memorable experience, roleplay
- Tone: practical, calm
- Keywords: reviews, delivery, return, customer service

## Asset 9. Domestic Trip or Staycation

- Summary: 강릉 같은 국내 여행이나 집에서 쉬는 휴가
- Reusable topics: vacation, planning, problem solving, preference
- Tone: restful
- Keywords: booking, packing, weather, itinerary

## Asset 10. Roleplay Reservation Fix

- Summary: 예약 변경, 일정 조정, 대안 제시
- Reusable topics: roleplay, cancellation, alternatives
- Tone: polite, helpful
- Keywords: reschedule, available, confirm, apologize
